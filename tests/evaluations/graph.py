...
class EvaluationConfig:
    ...
    # Evaluator model - using DeepSeek R1 for better reasoning
    EVALUATOR_MODEL = "siliconflow:deepseek-ai/DeepSeek-R1-0528-Qwen3-8B"
...
"""
    ...
    try:
        ...
        serializable_inputs, serializable_outputs = normalize_trajectory_data(trajectory)
...
async def test_graph_trajectory_evaluation(model_name, scenario):
    ...
    thread_id = f"eval_{model_name.replace(':', '_').replace('/', '_')}_{scenario['name']}"
    
    logger.info(f"Testing {model_name} with scenario '{scenario['name']}': {scenario['query']}")
    ...
    if "evaluation" in evaluation_result:
        ...
        logger.info(f"[{model_name}] [{scenario['name']}] Score: {score}, Reasoning: {reasoning}")
        assert isinstance(score, bool), f"Expected boolean score, got {type(score)}"
        assert score is True, f"Expected score to be True, got {score}. Reasoning: {reasoning}"